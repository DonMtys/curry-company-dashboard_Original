...
# Definir função de limpeza primeiro
def clean_code( df1 ):
    '''Esta funcao tem a responsabilidade de limpa o dataframe
    tipos de limpeza 
    1. removação dos dados Nan
    2. mudança do tipo da coluna de dados 
    3. removação do espaços das variaveis de texto 
    formatação de coluna de dadas '''
    ...
    # Limpar coluna multiple_deliveries
    linhas_selecionadas = (df1['multiple_deliveries'] != 'NaN ')
    df1 = df1.loc[linhas_selecionadas, :].copy()
    df1['multiple_deliveries'] = df1['multiple_deliveries'].astype(int)
    
    # Limpar time_taken
    df1['Time_taken(min)'] = df1['Time_taken(min)'].apply(lambda x: x.split('(min) ')[1] if '(min)' in str(x) else x)
